...
#define MEMORY_FREE_DEL(a)                 \
  {                                        \
    for (int t = 0; t < freeListIdx; t++)  \
    {                                      \
      if (freeList[t] && a == freeList[t]) \
      {                                    \
        free(freeList[t]);                 \
        freeList[t] = NULL;                \
        break;                             \
...
static bool session_init(bool session, bool restore)
{
  ...
  if (!restore)
  {
    ...
    if (session)
    {
      ...
      for (int i = 0; i < 9; i++)
      {
        if (i == 0)
        {
          if (main_ctx.sepStart != NULL)
            hex_encode((uint8_t *)main_ctx.sepStart, strlen(main_ctx.sepStart), (uint8_t *)f_sep[i]);
        }
        else if (i == 8)
        {
          if (main_ctx.sepEnd != NULL)
            hex_encode((uint8_t *)main_ctx.sepEnd, strlen(main_ctx.sepEnd), (uint8_t *)f_sep[i]);
        }
        else
        {
          int sepId = i - 1;
          if (main_ctx.sep[sepId] != NULL)
            hex_encode((uint8_t *)main_ctx.sep[sepId], strlen(main_ctx.sep[sepId]), (uint8_t *)f_sep[i]);
        }

#ifdef DEBUG
        if (debug)
          fprintf(stderr, "Encoded separator %d: %s\n", i, f_sep[i]);
#endif
      }
  ...
  {
    if (main_ctx.f[i])
      MEMORY_FREE_DEL(main_ctx.f[i])

    if (f_tmp[i] == NULL || !strcmp(f_tmp[i], "(null)"))
      continue;
  ...
  for (int i = 0; i < 9; i++)
  {
    ...
    if (i == 0)
    {
      if (main_ctx.sepStart)
        MEMORY_FREE_DEL(main_ctx.sepStart)

      if (f_sep[i] == NULL || !strcmp(f_sep[i], "(null)"))
        continue;
...
#ifdef DEBUG
      if (debug)
        fprintf(stderr, "Restore sepStart: '%s'\n", f_sep_tmp);
#endif
    ...
    {
      if (main_ctx.sepEnd)
        MEMORY_FREE_DEL(main_ctx.sepEnd);
...
#ifdef DEBUG
      if (debug)
        fprintf(stderr, "Restore sepEnd: '%s'\n", f_sep_tmp);
#endif
    ...
    {
      int sepId = i - 1;
      if (main_ctx.sep[sepId])
        MEMORY_FREE_DEL(main_ctx.sep[sepId]);
...
#ifdef DEBUG
      if (debug)
        fprintf(stderr, "Restore sep %d/%d: '%s'\n", sepId, i, f_sep_tmp);
#endif
  ...
  // restore sessionData
  // 1. Định nghĩa cấu trúc dữ liệu nhị phân (giống hệt lúc ghi trong session_update)
  struct
  {
    uint64_t cur_rep[8];
    uint64_t skip;
  ...
  // 2. Đọc trực tiếp 1 cục nhị phân từ ổ cứng lên RAM
  if (fread(&s_data, sizeof(s_data), 1, main_ctx.sfp[0]) != 1)
  {
    fprintf(stderr, "! fread(sessionData binary) failed\n");
    fclose(main_ctx.sfp[0]);
    main_ctx.sfp[0] = NULL;
    return false;
  }
...
#define EXIT_WITH_RET(_ret)      \
  {                              \
    if (main_ctx.sfp[0] != NULL) \
      fclose(main_ctx.sfp[0]);   \
                                 \
    if (main_ctx.fp[0] != NULL)  \
      fclose(main_ctx.fp[0]);    \
    if (main_ctx.fp[1] != NULL)  \
      fclose(main_ctx.fp[1]);    \
    if (main_ctx.fp[2] != NULL)  \
      fclose(main_ctx.fp[2]);    \
    if (main_ctx.fp[3] != NULL)  \
      fclose(main_ctx.fp[3]);    \
    if (main_ctx.fp[4] != NULL)  \
      fclose(main_ctx.fp[4]);    \
    if (main_ctx.fp[5] != NULL)  \
      fclose(main_ctx.fp[5]);    \
    if (main_ctx.fp[6] != NULL)  \
      fclose(main_ctx.fp[6]);    \
    if (main_ctx.fp[7] != NULL)  \
      fclose(main_ctx.fp[7]);    \
                                 \
    MEMORY_FREE_ALL              \
    return (_ret);               \
  }
...
int main(int argc, char *argv[])
{
  ...
  while ((opt = getopt_long(argc, argv, short_options, long_options, &long_index)) != -1)
  {
    ...
    case 0xb0:
      ...
      break;
    case '1':
      if (strlen(optarg) > 0 && access(optarg, F_OK) == 0)
      {
        main_ctx.f[0] = strdup(optarg);
        set++;
        MEMORY_FREE_ADD(main_ctx.f[0]);
      }
      else
        err++;
      break;
    case '2':
      if (strlen(optarg) > 0 && access(optarg, F_OK) == 0)
      {
        main_ctx.f[1] = strdup(optarg);
        set++;
        MEMORY_FREE_ADD(main_ctx.f[1]);
      }
      else
        err++;
      break;
    case '3':
      if (strlen(optarg) > 0 && access(optarg, F_OK) == 0)
      {
        main_ctx.f[2] = strdup(optarg);
        MEMORY_FREE_ADD(main_ctx.f[2]);
      }
      else
        err++;
      break;
    case '4':
      if (strlen(optarg) > 0 && access(optarg, F_OK) == 0)
      {
        main_ctx.f[3] = strdup(optarg);
        MEMORY_FREE_ADD(main_ctx.f[3]);
      }
      else
        err++;
      break;
    case '5':
      if (strlen(optarg) > 0 && access(optarg, F_OK) == 0)
      {
        main_ctx.f[4] = strdup(optarg);
        MEMORY_FREE_ADD(main_ctx.f[4]);
      }
      else
        err++;
      break;
    case '6':
      if (strlen(optarg) > 0 && access(optarg, F_OK) == 0)
      {
        main_ctx.f[5] = strdup(optarg);
        MEMORY_FREE_ADD(main_ctx.f[5]);
      }
      else
        err++;
      break;
    case '7':
      if (strlen(optarg) > 0 && access(optarg, F_OK) == 0)
      {
        main_ctx.f[6] = strdup(optarg);
        MEMORY_FREE_ADD(main_ctx.f[6]);
      }
      else
        err++;
      break;
    case '8':
      if (strlen(optarg) > 0 && access(optarg, F_OK) == 0)
      {
        main_ctx.f[7] = strdup(optarg);
        MEMORY_FREE_ADD(main_ctx.f[7]);
      }
      else
        err++;
      break;
